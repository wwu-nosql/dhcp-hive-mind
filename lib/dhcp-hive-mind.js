...
function DHCPHiveMind(opts) {
  ...
  this.handleDiscover = function(conn, req) {
    ...
    // Check which subnet request relay address belongs to
    for (var i = 0; i < self.config.subnets.length; i++) {
      ...
      if (block.contains(req.relay_ip)) {
        ...
        // Calculate subnets start and end address
        var start = block.first.split('.');
        var end = block.last.split('.');

        var found = false;

        // Select subnets lease database
        self.redisClient.select(i+1);

        // Loop through addresses, offer the first that isn't already
        // leased out
          ...
          if (!found) {
            ...
            self.redisClient.exists(ip, function(err, res) {
              if (!res && !found) {
                ...
                // Set lease in redis and set expiration of record
  ...
  this.handleRequest = function(conn, req) {
    ...
    // Loop through subnets to see which contains request relay ip
    for (var i = 0; i < self.config.subnets.length; i++) {
      var block = new Netmask(self.config.subnets[i].subnet);
      if (block.contains(req.relay_ip)) {
        var subnetMask = block.mask;
        ...
        // Select subnets database
        self.redisClient.select(i+1);

        // If there's a lease already, nack it else ack it
        self.redisClient.exists(req.client_ip, function(err, res) {
          ...
          // Set record in redis and expire it
